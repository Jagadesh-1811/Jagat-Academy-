...
const CourseDiscussion = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-black px-4 py-6 md:px-8">
      <div className="max-w-5xl mx-auto">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 text-sm font-semibold text-black hover:opacity-70 mb-4"
        >
          ...
          <div className="h-[65vh] overflow-y-auto px-4 md:px-6 py-5 space-y-4 bg-gray-50">
            {sortedMessages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-gray-500">
                ...
                <p className="text-sm mt-1">Start the first discussion for this course.</p>
              </div>
            ) : (
                  ...
                  <React.Fragment key={msg._id}>
                    {showDate && (
                      <div className="flex justify-center">
                        <span className="text-xs uppercase tracking-widest text-gray-500 bg-white border border-gray-200 rounded-full px-3 py-1">
                    ...
                    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[82%] md:max-w-[68%] rounded-3xl px-4 py-3 border shadow-sm ${isMe ? 'bg-black text-white border-black rounded-br-md' : 'bg-white text-black border-gray-200 rounded-bl-md'}`}>
                        <div className="flex items-center justify-between gap-3 mb-1">
                          <p className={`text-xs font-semibold ${isMe ? 'text-gray-300' : 'text-gray-500'}`}>
          ...
          <form onSubmit={sendMessage} className="border-t border-gray-200 bg-white p-4">
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 rounded-full border border-gray-300 bg-gray-50 px-4 py-3 text-sm outline-none focus:border-black"
                disabled={sending}
              />
              <button
                type="submit"

...
const EducatorLogin = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error("Educator Google Login Error:", error);
      if (error.code === 'auth/admin-restricted-operation') {
        ...
        toast.error(error.response?.data?.message || "Google login failed. Please register if you don't have an account.");
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${serverUrl}/api/admin/login`, form);
      localStorage.setItem('token', res.data.token);
      localStorage.setItem('user', JSON.stringify(res.data.user));
      toast.success('Logged in as Educator');
      navigate('/teacher/dashboard');
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8">
          <div className="border-b-4 border-black pb-4 mb-6">
            <h1 className="text-2xl font-black uppercase tracking-tight text-black">Educator Login</h1>
            <p className="text-gray-500 text-xs font-bold mt-1">Access your teaching dashboard</p>
          </div>
          ...
          <div className="mt-6 text-center space-y-2">
            <p className="text-gray-500 text-sm font-bold">
              Not registered?{' '}
              <Link to="/educator/signup" className="text-black underline hover:text-gray-600">
                Sign Up
              </Link>
            </p>

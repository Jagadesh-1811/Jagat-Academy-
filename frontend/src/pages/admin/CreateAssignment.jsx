...
const CreateAssignment = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post(`${serverUrl}/api/assignment/create/${courseId}`, 
                ...
                <div className="border-4 border-black p-6 bg-gray-50">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase tracking-wider text-gray-600">Assignment Title</label>
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase tracking-wider text-gray-600">Reference Link</label>
                            <input type="url" value={referenceLink} onChange={(e) => setReferenceLink(e.target.value)}
                                placeholder="https://..."
                                className="w-full border-2 border-black p-3 text-sm font-bold bg-white focus:outline-none" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase tracking-wider text-gray-600">Deadline</label>
